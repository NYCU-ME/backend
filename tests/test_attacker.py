...
def test_permission():
    ...
    response = requests.post(URL_BASE + "domains/me/nycu-dev/test-permission1")
    assert response.status_code == 401
    
    # Add record
    response = requests.post(URL_BASE + "ddns/me/nycu-dev/test-permission1/records/A/140.113.89.64")
    assert response.status_code == 401
    ...
    # Delete record
    response = requests.delete(URL_BASE + "ddns/me/nycu-dev/test-permission1/records/A/140.113.89.64")
    assert response.status_code == 401
    ...
    # Release domain
    response = requests.delete(URL_BASE + "domains/me/nycu-dev/test-permission1")
    assert response.status_code == 401
...
def test_invalid_operation():
    ...
    response = requests.post(URL_BASE + "domains/me/nycu-dev/test-attack", headers = headers)
    assert response.status_code == 200
    response = requests.post(URL_BASE + "ddns/me/nycu-dev/test-permission1/records/TXT/attack message%0Ans2 A 103.179.29.12")
